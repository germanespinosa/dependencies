...
macro (install_git_dependency DEPENDENCY_REPOSITORY)
    ...
    if (${DEPENDENCY_PUBLIC})
        set(DEPENDENCY_INCLUDE_SCOPE "PUBLIC")
    endif()
    if (${DEPENDENCY_PRIVATE})
        set(DEPENDENCY_INCLUDE_SCOPE "PRIVATE")
    endif()

    update_git_dependency(${DEPENDENCY_REPOSITORY} DEPENDENCY_FOLDER_NAME DEPENDENCY_HAS_UPDATES ${ARGN} )
    ...
    if (${DEPENDENCY_ADD_SUBDIRECTORY})
    ...
    elseif(${DEPENDENCY_CMAKE_PROJECT})
        ...
        if("${DEPENDENCY_IMPORT_TARGETS}" STREQUAL "")
            file(READ "${DEPENDENCY_DESTINATION}/dependency-targets.txt" DEPENDENCY_IMPORT_TARGETS)
        endif()

        foreach(DEPENDENCY_IMPORTED_TARGET ${DEPENDENCY_IMPORT_TARGETS})
            file(READ "${DEPENDENCY_DESTINATION}/dependency_${DEPENDENCY_IMPORTED_TARGET}-include_directories.txt" DEPENDENCY_IMPORTED_TARGET_INCLUDE_DIRECTORIES)
            if(NOT "${DEPENDENCY_IMPORTED_TARGET_INCLUDE_DIRECTORIES}" STREQUAL "")
                if("${DEPENDENCY_TARGET}" STREQUAL "")
                    include_directories(${DEPENDENCY_IMPORTED_TARGET_INCLUDE_DIRECTORIES})
                else()
                    target_include_directories(${DEPENDENCY_TARGET} ${DEPENDENCY_INCLUDE_SCOPE} ${DEPENDENCY_IMPORTED_TARGET_INCLUDE_DIRECTORIES})
                endif()
            endif()
            file(READ "${DEPENDENCY_DESTINATION}/dependency_${DEPENDENCY_IMPORTED_TARGET}-path.txt" DEPENDENCY_IMPORTED_TARGET_PATH)
            if("${DEPENDENCY_TARGET}" STREQUAL "")
                link_directories(${DEPENDENCY_IMPORTED_TARGET_PATH})
            else()
                target_link_directories(${DEPENDENCY_TARGET} ${DEPENDENCY_IMPORTED_TARGET_PATH})
            endif()
        endforeach()

        if(NOT "${DEPENDENCY_PACKAGES}" STREQUAL "")
            foreach(DEPENDENCY_PACKAGE ${DEPENDENCY_PACKAGES})
                set(${DEPENDENCY_PACKAGE}_DIR ${DEPENDENCY_DESTINATION}/${DEPENDENCY_FOLDER_NAME})
                find_package(${DEPENDENCY_PACKAGE})
            endforeach()
        endif()
    endif()

    if("${DEPENDENCY_INCLUDE_DIRECTORIES}" STREQUAL "")
        set(DEPENDENCY_HAS_INCLUDE_DIRECTORIES FALSE)
    else()
        set(DEPENDENCY_HAS_INCLUDE_DIRECTORIES TRUE)
    endif()

    if(${DEPENDENCY_HAS_INCLUDE_DIRECTORIES})
        foreach(DEPENDENCY_INCLUDE_DIRECTORY ${DEPENDENCY_INCLUDE_DIRECTORIES})
            if("${DEPENDENCY_TARGET}" STREQUAL "")
                include_directories(${DEPENDENCY_FOLDER}/${DEPENDENCY_INCLUDE_DIRECTORY})
            else()
                target_include_directories(${DEPENDENCY_TARGET} ${DEPENDENCY_INCLUDE_SCOPE} ${DEPENDENCY_FOLDER}/${DEPENDENCY_TARGET_INCLUDE_DIRECTORY})
            endif()
        endforeach()
    endif()
endmacro()
